...
/**
 * Adapts values whose runtime type may differ from their declaration type. This is necessary when a field's type is not the
 * same type that GSON should create when deserializing that field. For example, consider these types:
 * <pre>   {@code
 ...
 *     }
 *   }}</pre>
 * This class addresses this problem by adding type information to the serialized JSON and honoring that type information
 * when the JSON is
 ...
 * <h3>Registering Types</h3> Create a {@code RuntimeTypeAdapterFactory} by passing the base type and type field name to the
...
public final class RuntimeTypeAdapterFactory<T> implements TypeAdapterFactory {
    ...
    /**
     * Creates a new runtime type adapter using for {@code baseType} using {@code typeFieldName} as the type field name. Type
        ...
        return new TypeAdapter<R>() {
            ...
            public void write(JsonWriter out, R value) throws IOException {
                ...
                JsonObject jsonObject = delegate.toJsonTree(value)
                                                .getAsJsonObject();
                //                if (jsonObject.has(typeFieldName) && jsonObject.get(typeFieldName) == null) {
